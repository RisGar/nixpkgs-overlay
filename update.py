...
def update_file(filepath):
    ...
    for block_match in blocks:
        ...
        if is_commit:
            print(
                f"[{filepath}] {owner}/{repo}: Uses commit, checking latest commit..."
            )
            latest_rev = get_latest_commit(owner, repo)
        else:
            print(
                f"[{filepath}] {owner}/{repo}: Uses release ({rev}), checking latest release..."
            )
            latest_rev = get_latest_release(owner, repo)
            if not latest_rev:
                print(
                    f"[{filepath}] {owner}/{repo}: No release found, falling back to latest commit..."
                )
                latest_rev = get_latest_commit(owner, repo)
